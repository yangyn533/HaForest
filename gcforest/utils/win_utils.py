...
def get_windows_channel(X, X_win, des_id, nw, nh, win_x, win_y, stride_x, stride_y):
    ...
    src = X[:, int(k), int(di):int(di)+int(nh)*int(stride_y):int(stride_y), int(dj):int(dj)+int(nw)*int(stride_x):int(stride_x)].ravel()
    des = X_win[des_id, :]
    np.copyto(des, src)
...
def get_windows(X, win_x, win_y, stride_x=1, stride_y=1, pad_x=0, pad_y=0):
    ...
    X_win = np.empty(( int(nc), int(n) * int(nh) * int(nw)), dtype=np.float32)
    ...
    X_win = X_win.reshape((int(n), int(nh), int(nw), int(nc)))
    return X_win
